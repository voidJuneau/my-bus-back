...
express()
  ...
  schema = {
    // agency: "agency_id text,agency_name text,agency_url text,agency_timezone text,agency_lang text,agency_phone text,agency_fare_url text",
    // route: "route_id integer,agency_id text,route_short_name text,route_long_name text,route_desc text,route_type integer,route_url text,route_color text,route_text_color text",
    stop_time: "trip_id integer,arrival_time time,departure_time time,stop_id integer,stop_sequence integer,stop_headsign integer,pickup_type integer,drop_off_type integer,shape_dist_traveled float,agency_id text",
    ...
    try {
      ...
      for (const table of Object.keys(schema)) {
        ...
        for (let i = 1; i < data.length; i++) { // ignore the first line (header)
          ...
          let line = data[i].split(",").map(a => a.trim() === ""?"Null":`'${a.trim()}'`).join(",") // to allow empty value for integer
          console.log(header)
          console.log(line)
